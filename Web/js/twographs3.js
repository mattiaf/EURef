...
Two graphs on a plot http://www.d3noob.org/2013/07/arranging-more-than-one-d3js-graph-on.html
*/
...
// add the first graph canvas to the body of the webpage
...
// add the second graph canvas to the body of the webpage
...
d3.csv("data/GPDpeoplein3.csv", function(error, data) {
  ...
  data.forEach(function(d) { //strings to numeric
    d.Pop = +d.Pop/1000000;
    d.GDP = +d.GDP;
...
// don't want dots overlapping axis, add buffer to data domain
...
//make lines showing ratio of citizens to accepted refugees
...
var texts = svg.selectAll("textratio").data(ratioref).enter().append("svg:text")
							 ...
							 return stringrotate; // rotate label
							 })
...
svg.selectAll(".dot").data(data).enter().append("circle")
	.attr("class", "dot")
	.attr("id", function(d) { return d.GEO})  // id name of country
	.attr("r", 5.5)
	.attr("cx", PopMap)
	.attr("cy", PositiveMap)
	...
	.on("mouseover", function(d) {
			...
			var a = d3.selectAll('#'+name_dest) // select all dots with same id (also in the second graph!)...
			a.attr("r",18) // ..and make them bigger
	...
	.on("mouseout", function(d) {
	   ...
	   var name_dest = d3.select(this).attr("id") //go back to normal sie
...
svg2.selectAll(".dot")
	...
	.enter().append("circle")
	.attr("class", "dot")
	.attr("id", function(d) { return d.GEO})  // id is name of country
	.attr("r", 5.5)
